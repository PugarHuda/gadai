...
6. **Credit Line Board:** https://gadai-six.vercel.app/board. Every Bankr agent profile priced by the underwriting engine. Read live at 01:19 UTC on 2026-09-20 (`/api/board` `totals`): 105 Base agents, 14 eligible, **$856.57** of pre-approved credit, 593.3 WETH of lifetime fees, 19.5B LLM tokens in 30 days, plus 17 Robinhood Chain agents ($444.93 indicative) of which **3 earn their fees in tokenized stocks** (SPY, TSLA, MSTR; $64,514 of lifetime equity fees). These totals move with live fee data, so expect different numbers when you look.
...
8. **Slides:** https://gadai-six.vercel.app/deck (published from the repo alongside the video; if it 404s, the deck build has not deployed yet and everything it covers is on this page).
...
| Credit Line Board, incl. 17 Robinhood Chain agents (3 with tokenized-stock fees today: SPY, TSLA, MSTR) | Live reads; the Robinhood rows are indicative lines only, since the desk contracts are deployed on Base only | `/board` |
...
| Dynamic delegated access (auto-mirroring) | **Configured and live**: RSA credential uploaded, webhook verified by Dynamic and enforcing HMAC (unsigned POST → 401). No delegation has been granted yet, because that needs a human to approve the prompt in the browser | EVIDENCE "delegated access", [`agent/src/social/delegation.ts`](agent/src/social/delegation.ts) |
...
**Delegated access** (auto-mirroring for Follow the Desk). **Status: configured and live; awaiting one user approval.** The RSA delegation credential is uploaded in the Dynamic console, `DYNAMIC_AUTH_TOKEN` and `DYNAMIC_WEBHOOK_SECRET` are set, and the webhook `https://aqua-economic-moss-modes.trycloudflare.com/api/dynamic/webhook` is registered, verified by Dynamic and enforcing HMAC — an unsigned `POST` answers `401 {"error":"bad signature"}`. No delegation is stored yet, because granting one needs a human in a browser:
1. Open https://gadai-six.vercel.app/desk and log in with a Dynamic **embedded** wallet (delegation does not work for MetaMask-type external wallets).
2. In the follow form, tick **auto-mirror** and submit. The app calls `initDelegationProcess()` and Dynamic shows its delegation prompt.
3. Approve it. Dynamic posts `wallet.delegation.created` to the webhook with the encrypted key share; the agent verifies the HMAC, stores the ciphertext and logs `dynamic webhook: stored`.
4. From then on the desk signs that follower's mirror approvals and Flash orders itself, and the mirror shows the `auto · delegated` tag. Revoking in Dynamic posts `wallet.delegation.revoked` and the row is marked revoked.
...
- **Onchain equities (Robinhood Chain).** Every non-Base profile today is on Robinhood Chain (17 of them on 2026-09-20), and some of those pools are quoted in Robinhood tokenized stocks, so the creator's fees arrive as shares: EARN in SPY, TESLR in TSLA, MINR in MSTR (and SHOPKR in AMZN, whose profile fetch failed on the 2026-09-20 read). The board prices them with the same engine and lead persona, in quote units:
  ...
  - Live at 01:19 UTC on 2026-09-20: **3 equity-fee agents** (EARN in SPY, TESLR in TSLA, MINR in MSTR), **$64,514** of lifetime equity fees (beneficiary share) and **$164.73** of indicative equity credit, all of it EARN at $39.22/day of SPY fees. TESLR and MINR show `not eligible: fee rate is 0` because they had no recent claims. Across all 17 Robinhood agents, including WETH-quoted ones, indicative credit is $444.93. Quotes that minute: SPY $763.79, TSLA $365.04, MSTR $158.33. These numbers are live reads and move; a fourth agent (SHOPKR, AMZN) was on the board on 2026-09-19 and its fetch failed on the 2026-09-20 read (`totals.failed: 1`), which is exactly the behaviour the board documents. `/board` has an "Onchain equities" section with a stock-only filter.
