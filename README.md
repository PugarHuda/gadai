...
6. **Credit Line Board:** https://gadai-six.vercel.app/board. Every Bankr agent profile priced by the underwriting engine. Read live at 02:35 UTC on 2026-09-20 (`/api/board` `totals`): 105 Base agents, 14 eligible, **$869.86** of pre-approved credit, 593.3 WETH of lifetime fees, 19.6B LLM tokens in 30 days, plus 17 Robinhood Chain agents ($444.67 indicative) of which **4 earn their fees in tokenized stocks** (SPY, TSLA, MSTR, AMZN; $65,114 of lifetime equity fees), with `totals.failed: 0`. These totals move with live fee data, so expect different numbers when you look.
...
8. **Slides:** https://gadai-six.vercel.app/deck (published from the repo alongside the video; live, 200 at 02:3x UTC on 2026-09-20).
...
| Credit Line Board, incl. 17 Robinhood Chain agents (4 with tokenized-stock fees on the 02:35 UTC 2026-09-20 read: SPY, TSLA, MSTR, AMZN; the board drifts) | Live reads; the Robinhood rows are indicative lines only, since the desk contracts are deployed on Base only | `/board` |
| Buying a tokenized equity | **Mainnet, two chains.** 0.0002 ETH bridged Base → Robinhood Chain with Relay, then 0.00079 TSLA bought through the Uniswap Trading API on chain 4663 | EVIDENCE #6 |
| Dynamic delegated access (auto-mirroring) | **Granted by a real user.** A user approved the prompt from their embedded wallet `0x277aaE03…AFF9C`, Dynamic delivered the encrypted key share to the HMAC-verified webhook (unsigned POST → 401), the agent stored it at 02:10:49Z and the same user revoked it at 02:16:12Z — granted → delivered → stored → revoked, all observed server-side. The auto-mirror executor stays disabled while the desk runs on the fork, deliberately, so a grant cannot move funds here | EVIDENCE #7, [`agent/src/social/delegation.ts`](agent/src/social/delegation.ts) |
| Blackbird Flynet dining concierge | Live Flynet production data (app "hackathon 2" approved), incl. member OAuth login, membership/tag ranking and network check-in trending. No FLY moves: payments and rewards scopes are pending Blackbird review, and save-to-list answers an honest 501 because Blackbird has not published the endpoint | `/dine`, `/api/flynet/trending` |

### Per-track map

| Track | Requirement | Where it is satisfied | Evidence |
|---|---|---|---|
| **Bankr** | Build on Bankr | Fee APIs drive underwriting; `build-transfer-beneficiary` is the pledge; LLM Gateway memo path; Bankr Skill; x402 Cloud credit report; agent profile `gadai` (pending Bankr review). **Onchain equities:** the board prices Bankr agents whose fees arrive as tokenized stocks, and the desk bought one for real | [Bankr section](#bankr-grand-prize), [skill/gadai/SKILL.md](skill/gadai/SKILL.md), x402 402 above, EVIDENCE #6 |
| **Dynamic** | An agent makes a decision, then carries out a wallet/payment action | The Dynamic agent wallet (MPC, agent signing token) signs `createLoan`, the anchor bid and `disburse()` after the lead memo approves, pays for a risk verdict over x402 before approving, and tops itself up in USDC when its gas runs low. Delegated access for auto-mirroring is configured and enforcing webhook HMAC | [Dynamic section](#dynamic-an-agent-that-decides-then-pays), EVIDENCE #2–#6 |
| **Uniswap** | Use the Uniswap API / CCA; new assets, new agents; FEEDBACK.md | FeeNote ERC-20 sold in a CCA v2.1.0, agents price it; keeper swaps through the Trading API with the vault as swapper. **RWA:** the same Trading API prices and buys Robinhood tokenized stocks on chain 4663, which is how equity-quoted creator fees get valued and liquidated | [Uniswap section](#uniswap-new-assets-new-agents), [FEEDBACK.md](FEEDBACK.md), EVIDENCE #1 and #6 |
| **Definitive Flash** | Social trading build with a Flash advanced order; tag @DefinitiveFi | TWAP for the token leg, Follow the Desk leaderboard, bracket (TP/SL) and DCA mirrors, shareable signal cards, integrator fee set to 10 bps (`/api/flash/info`) | [Flash section](#definitive-flash-best-social-trading-build-definitivefi), EVIDENCE #5 |
| **Blackbird** | Build on Flynet | Dining concierge on live Flynet data (venues, hours, specials, challenges), member passport via OAuth, membership/tag ranking, network check-in trending, loan dining budget; `write:save_to_list` is granted but returns an honest 501 until Blackbird publishes the endpoint | [Blackbird section](#blackbird-flynet-dine-on-your-fees), `/dine` |
...
The dining concierge asks the Bankr LLM to order its verified Flynet shortlist ([`agent/src/flynet/index.ts#L710-L725`](agent/src/flynet/index.ts#L710-L725)); with no credits it says so and uses its deterministic ranker.
...
**Delegated access** (auto-mirroring for Follow the Desk). **Status: granted by a real user, then revoked by them — both observed server-side.** The RSA delegation credential is uploaded in the Dynamic console, `DYNAMIC_AUTH_TOKEN` and `DYNAMIC_WEBHOOK_SECRET` are set, and the webhook `https://aqua-economic-moss-modes.trycloudflare.com/api/dynamic/webhook` is registered, verified by Dynamic and enforcing HMAC — an unsigned `POST` answers `401 {"error":"bad signature"}`. On 2026-09-20 a user walked the flow end to end from the browser:
1. Opened https://gadai-six.vercel.app/desk and logged in with a Dynamic **embedded** wallet [`0x277aaE03…AFF9C`](https://basescan.org/address/0x277aaE03de02F5C9d33a6290Daba50E7792AFF9C) (delegation does not work for MetaMask-type external wallets).
2. In the follow form, ticked **auto-mirror** and submitted. The app calls `initDelegationProcess()` and Dynamic shows its delegation prompt.
3. Approved it. Dynamic posted `wallet.delegation.created` to the webhook with the encrypted key share; the agent verified the HMAC and stored the ciphertext (`HYBRID-RSA-AES-256`, decryptable only with the desk's RSA key) at **2026-09-20T02:10:49Z**, logging `dynamic webhook: stored`.
4. Revoked it from the Dynamic widget. Dynamic posted `wallet.delegation.revoked` and the row was marked revoked at **2026-09-20T02:16:12Z**.

With a live grant the desk signs that follower's mirror approvals and Flash orders itself, and the mirror shows the `auto · delegated` tag. The auto-mirror executor is kept **disabled while the desk runs on the fork** — a deliberate safety guard, not a missing piece: a grant here must not be able to move a real user's funds.

- [`agent/src/social/delegation.ts#L16-L95`](agent/src/social/delegation.ts#L16-L95): verifies the webhook HMAC (constant-time, raw or re-serialized body), replay-protects on `eventId` and event timestamp, stores only the **encrypted** share, and signs with `delegatedSignTypedData` / `delegatedSignTransaction`.
- [`agent/src/social/index.ts#L346-L361`](agent/src/social/index.ts#L346-L361): the auto executor. `POST /api/follows` with `auto:true` is refused with a 503 naming any missing env var, so auto mode can never silently degrade.
- [`web/app/desk/page.tsx#L202-L283`](web/app/desk/page.tsx#L202-L283): the follow form, which asks for delegation with `useWalletDelegation` and refuses to follow in auto mode if the status is not `delegated`.
...
**Integrator fee.** Every Flash order the desk sends carries the desk's integrator fee, set to **10 bps**. Check it live: `curl https://aqua-economic-moss-modes.trycloudflare.com/api/flash/info` → `{"integratorFeeBps":10,"integratorFeeSet":true,"mainnetOnly":true,"demoFork":true,"maxPriceImpactPct":3,"mirrors":{…}}`. `mainnetOnly` is why mirror counts are zero on the fork desk: Flash settles on mainnet, so the fork never places one.

**Shareable signals.** `GET /api/signals/:id` returns the card behind `/signals/:id` (persona, model, decision, the loan's on-chain repaid %, mirror counts), and `GET /api/signals/:id/flash-quote` prices the exact mirror a follower would place through Flash `/quote` — never `/order`.
...
- [`agent/src/flynet/index.ts#L84-L136`](agent/src/flynet/index.ts#L84-L136): the live catalog. All ~1,675 Blackbird locations (`GET /locations`, 34 pages, fetched sequentially) are cached for 6 h in memory and in SQLite. Hours are cached for 6 h and specials/challenges for 1 h. If Flynet fails, the last good copy is served and labelled stale. We use locations rather than `/restaurants` because many restaurant rows have empty names and no venue data.
- [`#L243-L333`](agent/src/flynet/index.ts#L243-L333): the request parser (city, neighborhood, cuisine, late, price, reservations, "somewhere new") and the catalog pre-ranker (one venue per brand, budget fit, distance when the browser shares its location).
- [`#L644-L707`](agent/src/flynet/index.ts#L644-L707): the plan. Live open hours for the top 12, then specials and challenges for the top 6, in small batches because Flynet rate-limits bursts (429 is retried once). Missing hours, failed reads and over-budget picks become notes; the result never hides them.
- [`#L710-L725`](agent/src/flynet/index.ts#L710-L725): the Bankr LLM orders the shortlist and writes one sentence per pick using only the given facts. With no credits the plan is labelled `deterministic`.
- [`#L344-L475`](agent/src/flynet/index.ts#L344-L475): member context through OAuth 2.0 + PKCE. The borrower signs `flynetLinkMessage` to bind the login to the loan. The server exchanges the code with `client_secret` and refreshes single-use tokens (one refresh per loan at a time). Then it reads the profile, status tier, wallets/FLY balance and check-ins. That "passport" shows places visited and gaps nearby (venues in the member's neighborhoods they have not tried), and it personalizes the plan. It is gated on `FLYNET_CLIENT_SECRET`: without it the UI says "member login needs the app secret". The secret is set on the live desk, so `/api/flynet/status` reports `memberLogin: {"available": true}`. Verified live on 2026-09-19: `/oauth/authorize` with our scopes 302s to `passport.flynet.org/authorize?flow_id=…`, a scope the app does not hold (`write:rewards`) is bounced back to our callback with `error=invalid_request`, and `/oauth/token` with a bogus code returns `400 invalid_grant`. Completing a login needs a real Blackbird member account; the click-path is in [docs/integrations/flynet.md](docs/integrations/flynet.md) §10.
- **Ranking signals beyond the request.** Network check-ins per venue over the last 7 days come from `/check_ins?location=&created_after=` (`pagination.total_count`, no member needed), a member's own membership cards add a reason and a bonus, and industry tags only ever add a reason, because Flynet tags are `type:industry` and not tastes. `GET /api/flynet/trending?region=` ranks the venues in the latest 500 network check-ins by that 7-day count; it is a card on `/dine`.
- **Save to a Blackbird list: honest 501.** The app holds `write:save_to_list`, but no endpoint for it exists in the Flynet OpenAPI, SDK 0.8.1, MCP, skills or docs (checked 2026-09-19). `POST /api/loans/:id/dine/save` validates the session and the scope and then returns **501** with that reason, and the web button is disabled and says so. We did not guess a path.
- [`#L744-L860`](agent/src/flynet/index.ts#L744-L860): routes. `GET /api/flynet/status`, `GET /api/flynet/trending?region`, `GET /api/flynet/restaurants?query&region&cuisine&price&page&loanId`, `GET /api/flynet/restaurants/:id`, `GET /api/loans/:id/dine`, `POST /api/loans/:id/dine/plan {request, partySize, time?, near?}`, `GET /api/loans/:id/dine/passport`, `POST /api/loans/:id/dine/save` (501) and `DELETE /api/loans/:id/dine/member` (member session in the query string, because the server's CORS allowlist has no custom header), `GET /api/flynet/connect` and `/callback`. Bad input returns 400. Live: `curl https://aqua-economic-moss-modes.trycloudflare.com/api/flynet/status` names the app and its granted scopes.
...
- **Onchain equities (Robinhood Chain).** Every non-Base profile today is on Robinhood Chain (17 of them on 2026-09-20), and some of those pools are quoted in Robinhood tokenized stocks, so the creator's fees arrive as shares: EARN in SPY, TSLR in TSLA, MINR in MSTR and SHOPKR in AMZN. The board prices them with the same engine and lead persona, in quote units:
  ...
  - Live at 02:35 UTC on 2026-09-20: **4 equity-fee agents** (EARN in SPY, TSLR in TSLA, MINR in MSTR, SHOPKR in AMZN), **$65,114** of lifetime equity fees (beneficiary share) and **$164.63** of indicative equity credit, all of it EARN at $39.20/day of SPY fees. TSLR, MINR and SHOPKR show `not eligible: fee rate is 0` because they had no recent claims. Across all 17 Robinhood agents, including WETH-quoted ones, indicative credit is $444.67, and every profile fetched cleanly (`totals.failed: 0`). Quotes that minute: SPY $763.73, TSLA $364.79, MSTR $157.77, AMZN $253.93. These are live reads and drift through the day — the count and the totals both move as profiles and quotes change. `/board` has an "Onchain equities" section with a stock-only filter.
...
### Onchain equities: a real tokenized-stock purchase (EVIDENCE #6)
**Done, on mainnet.** The desk bridged 0.0002 ETH from Base to Robinhood Chain and bought **0.00078962 TSLA** for 0.000109976 ETH: bridge [`0xc65bd93a…5af2d4`](https://basescan.org/tx/0xc65bd93a23bdf9756dec9503aeb40ff902d9beeb53200a6068c99cab2e5af2d4) on Base, swap [`0xbfbe9702…66dd708`](https://robinhoodchain.blockscout.com/tx/0xbfbe9702dd40ed28e734d1ebc319a7ace9d27b30f77eb5185179de01366dd708) on 4663. That closes the loop the board opens: equity-quoted creator fees are priced through the Trading API on 4663, and this proves the same API will actually trade them.
...
- **Just-in-time USDC top-up.** With `RISK_AUTO_TOPUP=1`, if the desk's Base USDC is below the price when a verdict is due, the wallet first swaps `max(price × 1.2, $0.10)` of USDC for native ETH through the Uniswap Trading API (EXACT_OUTPUT) so the payment can go through, then pays. It is capped twice (`RISK_TOPUP_MAX_USDC_PER_DAY`, default 0.5, and `RISK_TOPUP_MAX_ETH`, default 0.0001 per tx), recorded in `kv` (`risk.topup.last`, `risk.topupRaw.<day>`), linked from the `RiskCheck` note, refused unless the swap target is a Universal Router 2.0 or SwapProxy address, and disabled in `DEMO_FORK`. Off by default; see `.env.example`.
- **API.** `GET /api/risk/:token` returns the cached verdict and never spends — on the live desk it currently answers `{"verdict":null,"note":"no verdict bought in the last 24h","spentTodayRaw":"0"}` for a token nobody has queried today. `POST /api/admin/risk/:token` (`x-admin-token`) forces a paid purchase, still under the budget.
