...
- Demo video (3:49, narrated): https://youtu.be/F7joLyWWB0E (also at https://gadai-six.vercel.app/demo · direct MP4: https://gadai-six.vercel.app/gadai-demo.mp4)
- Slides: https://gadai-six.vercel.app/deck (live, 200 at 02:3x UTC on 2026-09-20)
...
**Also built:**
- Credit Line Board: every Bankr agent profile priced live by the same engine. At 02:35 UTC on 2026-09-20: 105 Base agents, 14 eligible, $869.86 of pre-approved credit, 593.3 WETH lifetime fees, 19.6B LLM tokens in 30 days. It also lists 17 Robinhood Chain agents as indicative lines ($444.67), 4 of them earning fees in tokenized stocks (SPY, TSLA, MSTR, AMZN; $65,114 of lifetime equity fees), with no failed profile fetches. These are live reads and drift through the day.
...
**Honest scope:** the full loan cycle ran end to end on an Anvil fork of Base against a real Bankr pool (GITLAWB): apply, pledge, CCA, 239.75 USDC disbursed, 0.166 WETH collected, Uniswap Trading API swap to 289.71 USDC, repaid, released, ERC-8004 feedback. Our Bankr LLM credits are $0, so memos are written by deterministic persona rules and labelled "rules, no LLM review"; the LLM path is built and fails closed. The vault-funded Flash TWAP and follower mirrors are built and quote-tested but not filled (Flash settles on mainnet only). The Flynet app is approved on production with read scopes plus `write:save_to_list`; payment and rewards scopes are pending Blackbird review, so no FLY moves, and save-to-list returns an honest 501 because Blackbird has not published that endpoint. Dynamic delegated access was granted for real: a user approved the prompt from their embedded wallet, Dynamic delivered the encrypted key share to the HMAC-verified webhook (unsigned posts still get 401), the agent stored it and the user then revoked it — all observed server-side on 2026-09-20. The auto-mirror executor itself stays disabled while the desk runs on the fork, deliberately, so a grant cannot move funds here.
