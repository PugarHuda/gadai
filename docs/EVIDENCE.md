...
Reproduce: `agent/src/demo/mainnet-deploy.ts` (1–3), `agent/src/demo/risk-check.ts` (4) and `agent/src/demo/desk-mirror.ts` (5), run under WSL/Linux with the `.env` described in `.env.example`.
