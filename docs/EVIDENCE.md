...
| 7 | **Dynamic delegated access, granted by a user**: from the browser, a user with the embedded wallet [`0x277aaE03de02F5C9d33a6290Daba50E7792AFF9C`](https://basescan.org/address/0x277aaE03de02F5C9d33a6290Daba50E7792AFF9C) approved "hackathon is requesting access to perform automations on your behalf". Dynamic delivered the encrypted delegated key share to the desk's webhook (HMAC-verified; an unsigned POST returns 401) and the agent stored it (`delegations` row, `HYBRID-RSA-AES-256`, decryptable only with the RSA key the desk generated) at 2026-09-20T02:10:49Z. The same user then revoked it from the Dynamic widget and the desk logged `dynamic webhook: revoked` at 2026-09-20T02:16:12Z — granted, delivered, revoked, all observed on our side | Dynamic track: "an assistant that acts for a user within their approved wallet permissions" — user-owned wallet, scoped, and revocable from the same widget | webhook event stored server-side (no chain tx; the grant lives in Dynamic + our vault). Safety: the auto-mirror executor is disabled while the desk runs on the fork, so the grant cannot move funds here |

Rows 1–5 are Base mainnet. Row 6 is one Base tx (the bridge deposit) plus one Robinhood Chain tx (the swap). Row 7 has no chain tx: the grant lives in Dynamic and our vault, and the webhook events are logged server-side.
...
- **Dynamic delegated access: granted by a user, then revoked (row 7).** The RSA credential is uploaded in the Dynamic console, `DYNAMIC_AUTH_TOKEN` and `DYNAMIC_WEBHOOK_SECRET` are set, and the webhook endpoint is verified by Dynamic and enforcing HMAC. Check the HMAC yourself: an unsigned POST is rejected.
  ```bash
  curl -s -o /dev/null -w '%{http_code}\n' -X POST -H 'content-type: application/json' -d '{}' \
    https://aqua-economic-moss-modes.trycloudflare.com/api/dynamic/webhook     # → 401 (bad signature)
  ```
  A person then walked it: opened `/desk`, ticked **auto-mirror** and approved the Dynamic prompt, which made Dynamic post `wallet.delegation.created` (stored 2026-09-20T02:10:49Z) and later `wallet.delegation.revoked` (2026-09-20T02:16:12Z). Row 7 is that delegation. The auto-mirror executor stays disabled while the desk runs on the fork, on purpose. Steps: [docs/integrations/dynamic.md §3](integrations/dynamic.md).
